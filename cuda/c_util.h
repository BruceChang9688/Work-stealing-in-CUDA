...
__device__
    Color
    ambientColor(const Color &color)
{
...
__device__
    Color
    specularColor(const Vector3D &direction, const Vector3D &normal,
...
__device__
    Color
    diffuseColor(const Vector3D &direction, const Light *light,
...
__device__
    intersection_type
    calculateIntersect(const Ray &ray,
                       ...
                       int depth)
{
  ...
  do
  {
    ...
    if (type != NONE)
    {
      float intersection_reflection = type == PLANE ? i_plane.reflection() : i_sphere.reflection();
      ...
      for (int i = 0; i < num_lights; i++)
      {
        ...
        {
          float diffuse_coefficient = type == PLANE ? i_plane.diffuse() : i_sphere.diffuse();

          float specular_coefficient = type == PLANE ? i_plane.specular() : i_sphere.specular();
...
__global__ void k_trace(Image *d_image,
                        Plane *d_planes, int num_planes,
                        Sphere *d_spheres, int num_spheres,
                        Light *d_lights, int num_lights,
                        ...
                        int numRay, float portion, int capacity, int *record)
{
  ...
  if (x < width && y < height)
  {
    ...
    while (1)
    {
      ...
      if (status != QueueStatus::QUEUEISWORKING)
      {
        break;
      }
...
__global__ void k_trace_without_work_stealing(Image *d_image,
                                              Plane *d_planes, int num_planes,
                                              Sphere *d_spheres, int num_spheres,
                                              Light *d_lights, int num_lights,
                                              float aspect_ratio, float tanFov,
...
__global__ void init_stuff(unsigned int seed, float *state)
{
